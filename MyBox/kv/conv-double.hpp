...
struct conv_double {
	...
	static double stringtod(std::string s, int mode = 0) {
		...
		for (long unsigned int l=0; l<num1_s.size(); l++) {
			table[offset + num1_s.size() - 1 - l + e10] = num1_s[l] - '0';
		}

		for (long unsigned int l=0; l<num2_s.size(); l++) {
			table[offset - l - 1 + e10] = num2_s[l] - '0';
		}
